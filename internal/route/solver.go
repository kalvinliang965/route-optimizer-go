...
func Solve(stops []Stop, matrix [][]float64, k int) ([]RouteResult, error) {
	...
	if n == 0 {
		return nil, fmt.Errorf("cannot solve a route with no stops")
	}
	if n == 1 {
		return []RouteResult{{Path: []int{0, 0}, Duration: 0.0}}, nil
	}
	...
	for i := 0; i < len(results); i++ {
		// A complete tour starts and ends at index 0 (the depot).
		fullRoute := make([]int, 0, len(results[i])+2)
		fullRoute = append(fullRoute, 0)
		fullRoute = append(fullRoute, results[i]...)
		fullRoute = append(fullRoute, 0)
		duration := 0.0
