...
func TestEdgeMetadataRoundTrip(t *testing.T) {
	...
	assertEdgeEndpoints(t, got.Edges[0], 0, 1,
		optimizer.Stop{Lat: want.Edges[0].Geometry[0].Lat, Lon: want.Edges[0].Geometry[0].Lon},
		optimizer.Stop{Lat: want.Edges[0].Geometry[1].Lat, Lon: want.Edges[0].Geometry[1].Lon},
	)
...
func fixtureStops() []optimizer.Stop {
...
func readOSRMRouteFixture(t *testing.T) []byte {
	...
	if err != nil {
		t.Fatalf("read OSRM fixture: %v; regenerate with PEPSI_RECORD_OSRM_FIXTURE=1 go test ./internal/deprecated/traffic -run TestRecordOSRMRouteFixture", err)
	}
	return body
}
...
func metersBetween(c Coordinate, s optimizer.Stop) float64 {
...
func assertEdgeEndpoints(t *testing.T, edge EdgeMetadata, fromIdx, toIdx int, from, to optimizer.Stop) {
	t.Helper()
...
func osrmRouteJSON(from, to optimizer.Stop, stepName string) string {
	return fmt.Sprintf(`{
		"code": "Ok",
		"routes": [{
			"duration": 420.5,
