// Package traffic preserves the stalled OSRM and NYC DOT traffic experiment.
//
// Deprecated: this package is not part of the active planner or server path.
package traffic
...
func (c Client) BuildEdgeMetadata(ctx context.Context, stops []optimizer.Stop) (EdgeMetadataFile, error) {
...
func WriteEdgeMetadata(path string, metadata EdgeMetadataFile) error {
	return storage.WriteJSON(path, metadata)
}

func ReadEdgeMetadata(path string) (EdgeMetadataFile, error) {
	var metadata EdgeMetadataFile
	if err := storage.ReadJSON(path, &metadata); err != nil {
		return EdgeMetadataFile{}, fmt.Errorf("read edge metadata %s: %w", path, err)
	}
	return metadata, nil
}

func (c Client) FetchEdge(ctx context.Context, stops []optimizer.Stop, fromIdx, toIdx int) (EdgeMetadata, error) {
...
func (c Client) fetchEdgeBody(ctx context.Context, stops []optimizer.Stop, fromIdx, toIdx int) ([]byte, error) {
